...
// cross a line break.
//
// The same token is used for dimension declarations (`input t : мм`) and unit
...
static bool is_keyword(const char *word, int len) {
  static const char *const keywords[] = {
      "if",     "then",  "elseif", "else",  "end",   "begin", "and",
      "or",     "not",   "mod",    "nil",   "true",  "false", "where",
      "match",  "case",  "const",  "in",    "unit",  "input", NULL,
  };
  for (int i = 0; keywords[i] != NULL; i++) {
    if ((int)strlen(keywords[i]) == len && strncmp(keywords[i], word, len) == 0) {
      return true;
    }
  }
  return false;
