...
module.exports = grammar({
  ...
  rules: {
    ...
    // `input name : unit : description = value`
    ...
    input_declaration: $ => seq(
      ...
      optional(
        seq(
          ':',
          choice(
            seq(':', optional(field('description', $.description))),
            seq(
              field('unit', $.unit),
              optional(seq(':', optional(field('description', $.description)))),
            ),
          ),
        ),
      ),
    ...
    // `const name : unit : description = value` (the second `:` is required:
    // without it the construct falls through to a const_assignment with a
    // description). `prec(3)` puts the declaration before the assignment,
    // matching the reference grammar's rule order.
